...
BEGIN
	...
	CASE    
	WHEN    @holiday LIKE N'Нов%'
	THEN    dbo.GetNewYearHolidaysStartDate(@year)
	WHEN    @holiday LIKE N'Пасха'
	THEN    dbo.GetEasterDate(@year)
	WHEN    @holiday LIKE N'Летние%'
	THEN    dbo.GetSummerHolidaysStartDate(@year)
	ELSE    DATEFROMPARTS
			(@year, 
